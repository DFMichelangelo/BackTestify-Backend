...
export default function initializeRoutes(router: Router): void {
	...
	//res.redirect(301, process.env.WEBAPP_FRONTEND_URL)
	//})

	// ? Echo route
	router.get("/", (req: Request, res: Response, next: NextFunction) => {
		let htmlContent = `<h1>${config.longTitle}</h1>`;
		if (!isProduction) htmlContent += `<h2><a href=\"${config.apiDocsLink}\">API Documentation</a></h2>`
